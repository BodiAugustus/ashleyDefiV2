export default function AshleyAdv({}) {
  return (
    <div
      className="w-[100vw] pt-20 
        xs:h-auto"
    >
      <h2
        className="font-bold text-white text-center mb-5
        xs:text-2xl xs:mb-1 
        xs4:text-3xl
        md:text-4xl 
      "
      >
        The Ashley Advantage
      </h2>
      <p
        className="text-[#B0C7EF] text-center  mx-auto
        xs:text-sm xs:mb-4 xs:w-[95%] 
        xs5:text-base 
        xs6:text-lg xs6:leading-6 
        md:text-xl md:mb-8
        lg:w-[60%] 
        xl:w-[50%]  
     
      "
      >
        Partnering with Ashley Capital & DeFi means our consultants will
        dedicate themselves fully to helping you achieve your financial goals
      </p>
      <div className="flex flex-col gap-5">
        <div>
          <div
            className="flex flex-col mx-auto p-4  white-glassmorphism text-white
          xs:gap-1 xs:w-[95%]
          md:gap-4 md:w-[90%]
"
          >
            <h3
              className="font-bold
            xs:text-xl 
            xs5:text-2xl
            md:text-2xl  
           "
            >
              Collaboration
            </h3>
            <p
              className="text-[#B0C7EF] 
            xs:text-sm 
            xs5:text-base 
            xs6:text-lg
            md:text-lg 
           "
            >
              We emphasize the importance of working closely with our clients as
              partners, not just as service providers. This involves open
              dialouge, timely communication, and a deep understanding of our
              clients individual needs and aspirations in order to create real
              and tangible value.
            </p>
          </div>
        </div>
        <div>
          <div
            className="flex flex-col mx-auto p-4 white-glassmorphism text-white
          xs:gap-1 xs:w-[95%] 
          md:gap-4 md:w-[90%]  
          "
          >
            <h3
              className="font-bold
            xs:text-xl 
            xs5:text-2xl
            md:text-2xl  "
            >
              Integrity
            </h3>
            <p
              className="text-[#B0C7EF] 
            xs:text-sm  
            xs5:text-base 
            xs6:text-lg
            md:text-lg"
            >
              Our consultants uphold the highest standards of honesty and
              ethical behavior. In the context of DeFi, where trust and security
              are paramount, integrity means providing reliable and truthful
              information always while simultaneously advocating for the best
              interest of the client, especially in times of volatility.
            </p>
          </div>
        </div>
        <div>
          <div
            className="flex flex-col mx-auto p-4  white-glassmorphism text-white
          xs:gap-1 xs:w-[95%]
          md:gap-4 md:w-[90%]"
          >
            <h3
              className="font-bold 
            xs:text-xl 
            xs5:text-2xl
            md:text-2xl "
            >
              Returns
            </h3>
            <p
              className="text-[#B0C7EF] 
            xs:text-sm  
            xs5:text-base 
            xs6:text-lg
            md:text-lg"
            >
              Our consultants concentrate on strategies that maximize our client
              returns and profitability while mitigating risk and exposure. This
              involves identifying and implementing investment opportunities
              within the DeFi sector that offer the most optimal returns in
              order to help ensure our clients' financial success.
            </p>
          </div>
        </div>
        <div></div>
      </div>
    </div>
  );
}
